...
newtype CollectedStates loc =
    CollectedStates
    { unpackCollectedStates :: Map loc [[Either Integer Integer]] }
...
runRsm oldRsmState loc partState onlyNew onlyOld =
    do (newRsm, newPreds) <-
           case (onlyNew, onlyOld) of
             ...
             (False, True) ->
                 do (newRsm1, rsm1Preds) <- runOldRSM
                    return $ (RSM collectedStates newRsm1 emptyRSM2, Set.singleton rsm1Preds)
             (True, False) ->
                 let (newRsm2, rsm2Preds) = runNewRSM
                 in return $ (RSM collectedStates emptyRSM1 newRsm2, rsm2Preds)
             _ ->
                 do (newRsm1, rsm1Preds) <- runOldRSM
                    let (newRsm2, rsm2Preds) = runNewRSM
                    return $ (RSM collectedStates newRsm1 newRsm2, Set.insert rsm1Preds rsm2Preds)
    ...
    where
      ...
      collectedStates = rsm_collectedStates oldRsmState
